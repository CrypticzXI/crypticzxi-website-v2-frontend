...
const Portfolio = ({
  ...
  const limitString = (post) => {
    ...
    if (10 < len) {
      const truncated = post.slice(0, 80);
            ...
            <div className='flex flex-col w-full h-full mt-20'>
              ...
              <div className="w-full flex flex-col h-full z-[999] space-y-8">
                ...
                <a href={`/blog/${blog.slug.current}`} key={blog._key} className='w-full lg:h-40 h-[30rem] bg-secondarybackground rounded-xl flex flex-col lg:flex-row shadow-xl overflow-hidden'>
                  <div className='IMAGE-AREA w-full lg:w-96 lg:h-full h-[15rem] overflow-hidden flex items-center justify-center bg-center bg-cover' style={{backgroundImage: `url(${urlFor(blog.image)})`}}>
                  </div>

                  <div className='TEXT-AREA p-8 flex flex-col'>
                    <h2 className='flex space-x-3 lg:text-2xl font-poppins uppercase text-secondary font-[700] text-lg xl:text-xl'>{blog.title}</h2>
                    <p className='font-rubik text-white mt-2 break-words text-sm xl:text-base'>{limitString(toPlainText(blog.post))}</p>
