            ...
            <div className='grid lg:grid-cols-2 lg:grid-rows-2 w-full gap-8 grid-rows-4 grid-cols-1 z-[999] py-12'>
                ...
                {portfoliodata.map((portfolio) => (
                    <a href={`/portfolio/${portfolio.slug.current}`} className='w-full aspect-video rounded-lg border-background border-2 ring-2 tracker ring-secondarybackground group overflow-hidden relative'>
                        ...
                        <div className="transition-all w-full bg-center bg-cover aspect-video" alt={portfolio.title} style={{ backgroundImage: `url(${urlFor(portfolio.thumbnail.image)})` }}></div>
                    </a>
                ))}
            </div>
        </div>
